...
public class Trawa extends Roślina {
    ...
    public Trawa(int sila, int inicjatywa, int x, int y, SwiatInterface swiat, String znak) {
    ...
    public void kolizja(Organizm inny) {
        ...
        if (inny.getX() == this.getX() && inny.getY() == this.getY()) {
            if (this.getSila() > inny.getSila()) {
                String log = "TRAWA ZABIŁA " + inny.getNazwa();
                swiat.dodajDoDziennika(log);
                inny.setZyje(false);
                swiat.usunOrganizm(inny);
            } else {
                String log = inny.getNazwa() + " ZJADŁ TRAWĘ";
                swiat.dodajDoDziennika(log);
                this.setZyje(false);
